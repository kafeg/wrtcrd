...
type RemoteScreenPeerConn struct {
	...
	grabber    rdisplay.ScreenGrabberInterface
	encService encoders.ServiceInterface
}

func findBestCodec(sdp *sdp.SessionDescription, encService encoders.ServiceInterface, h264Profile string) (*webrtc.RTPCodec, encoders.VideoCodec, error) {
...
func newRemoteScreenPeerConn(stunServer string, grabber rdisplay.ScreenGrabberInterface, encService encoders.ServiceInterface) *RemoteScreenPeerConn {
...
func (p *RemoteScreenPeerConn) ProcessOffer(strOffer string) (string, error) {
	...
	peerConn.OnICEConnectionStateChange(func(connState webrtc.ICEConnectionState) {
		...
		if connState == webrtc.ICEConnectionStateDisconnected || connState == webrtc.ICEConnectionStateFailed {
			p.Close()
			log.Fatal("Unable to establish connection. Exit...")
		}
	})
